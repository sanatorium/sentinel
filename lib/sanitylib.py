...
def is_valid_sanity_address(address, network='mainnet'):
    ...
    sanity_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Sanity address
        return False

    if (address_version != sanity_version):
        return False
...
# shims 'til we can fix the sanityd side
def SHIM_serialise_for_sanityd(sentinel_hex):
    from models import SANITYD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SANITYD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in sanityd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sanityd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sanityd_hex = serialise(obj)
    return sanityd_hex


# shims 'til we can fix the sanityd side
def SHIM_deserialise_from_sanityd(sanityd_hex):
    from models import SANITYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sanityd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('sanity.conf')
