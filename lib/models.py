...
class GovernanceObject(BaseModel):
    ...
    # sync sanityd gobject list with our local relational DB backend
    @classmethod
    def sync(self, sanityd):
        golist = sanityd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_sanityd(sanityd, item)
    ...
    @classmethod
    def import_gobject_from_sanityd(self, sanityd, rec):
        import decimal
        import sanitylib
        import inflection
        ...
        # shim/sanityd conversion
        object_hex = sanitylib.SHIM_deserialise_from_sanityd(object_hex)
        objects = sanitylib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from sanityd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from sanityd, with every run
        ...
        # get/create, then sync payment amounts, etc. from sanityd - Sanityd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(sanityd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from sanityd! %s" % e)
            govobj.vote_delete(sanityd)
            return (govobj, None)
    ...
    def vote_delete(self, sanityd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(sanityd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, sanityd, signal, outcome):
        import sanitylib
        ...
        output = sanityd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = sanitylib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(sanityd, signal)

    def sync_network_vote(self, sanityd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = sanityd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SANITYD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 sanity addr, non-multisig
            if not sanitylib.is_valid_sanity_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Sanity address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (sanitylib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SanityDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import sanitylib
        obj_data = sanitylib.SHIM_serialise_for_sanityd(self.serialise())
    ...
    def prepare(self, sanityd):
        try:
            object_hash = sanityd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SANITYD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not sanitylib.is_valid_sanity_address(addr, config.network):
    ...
    def hash(self):
        import sanitylib
        return sanitylib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SANITYD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, sanityd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - sanityd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, sanityd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - sanityd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, sanityd):
        now = int(time.time())
        return (self.created_at < (now - sanityd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, sanityd):
        if self.is_expired(sanityd):
            return False

        return True

    def is_deletable(self, sanityd):
        if self.is_expired(sanityd):
            return True
