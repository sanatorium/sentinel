...
def has_sanity_conf():
    ...
    valid_sanity_conf = False

    # ensure sanity_conf exists & readable
    #
    # if not, print a message stating that Sanity Core must be installed and
    # configured, including JSONRPC access in sanity.conf
    try:
        f = io.open(config.sanity_conf)
        valid_sanity_conf = True
    except IOError as e:
        print(e)

    return valid_sanity_conf
...
def main():
    ...
    if not has_sanity_conf():
        print("SanityCore must be installed and configured, including JSONRPC access in sanity.conf")
        sys.exit(1)
