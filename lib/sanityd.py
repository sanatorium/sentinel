...
class SanityDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some sanityd methods
    ...
    @classmethod
    def from_sanity_conf(self, sanity_dot_conf):
        from sanity_config import SanityConfig
        config_text = SanityConfig.slurp_config_file(sanity_dot_conf)
        creds = SanityConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = sanitylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = sanitylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = sanitylib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_sanityd_proto_version_with_sentinel_ping)
